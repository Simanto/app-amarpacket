...
const TableMerchantReturned = () =>{

  const {getPacket,setEditPacket,getPacketReturned, packetsReturned} = useAppContext();
  ...
  useEffect(() => {
    ...
    return (
      <div className="table">
        {packetsReturned ? 
            <ElementTable columns={columns} initialState={initialState} data={packetsReturned} filterCmponents={["search"]} />
            :
            <Loading />
          }
      </div>
...
export default TableMerchantReturned